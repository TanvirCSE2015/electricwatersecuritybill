...
<div class="container">
    <div class="gw-card">
        ...
        <form id="gatewayForm" method="POST" action="{{ route('water.bill.process') }}">
            @csrf
            <input type="hidden" name="holding_number" value="{{ $holdingNumber }}">
            <input type="hidden" name="payment_method" id="selectedMethod" value="bKash">
            <input type="hidden" name="previous_due_paid" value="{{ $previousDuePaid }}">
            <input type="hidden" name="s_previous_due_paid" value="{{ $sPreviousDuePaid }}">
            @foreach($billIds ?? [] as $id)
                <input type="hidden" name="bill_ids[]" value="{{ $id }}">
            @endforeach
            @foreach($securityIds ?? [] as $id)
                <input type="hidden" name="security_ids[]" value="{{ $id }}">
            @endforeach
            ...
            <div class="gw-body">
                <div class="gw-row">
                    ...
                    <span class="value">{{ $customer->customer_name ?? $customer->name ?? '' }}</span>
                </div>
                <div class="gw-row">
                    <span class="label">Holding No</span>
                    <span class="value">{{ $customer->holding_number }}</span>
                </div>
                <div class="gw-row">
            ...
            <div class="gw-footer">
                ...
                <a href="{{ route('water.bill.index') }}" class="gw-cancel">← Cancel / Back</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // 3. Call /water-bill/process-payment to create invoices
